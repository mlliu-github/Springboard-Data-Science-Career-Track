...
SELECT COUNT( name ) AS "Facility Number"
FROM `Facilities`
WHERE membercost = 0.0
...
SELECT name, 
       ...
       CASE WHEN monthlymaintenance > 100 THEN 'expensive'
            ELSE 'cheap' END AS "Facility Price Category"
FROM `Facilities`
...
the member name. */
SELECT DISTINCT `Facilities`.name, CONCAT( `Members`.surname, ', ', `Members`.firstname ) AS "Member Name"
FROM `Bookings`
JOIN `Facilities` ON ( `Facilities`.facid = `Bookings`.facid )
JOIN `Members` ON ( `Members`.memid = `Bookings`.memid )
WHERE `Facilities`.name LIKE '%Tennis Court%'
ORDER BY CONCAT( `Members`.surname, ', ', `Members`.firstname )
...
WHERE (`Members`.memid = 0 and `Bookings`.slots * `Facilities`.guestcost > 30) or
	  (`Members`.memid != 0 and `Bookings`.slots * `Facilities`.membercost > 30)
ORDER BY 3 DESC
